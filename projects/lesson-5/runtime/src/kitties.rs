...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn set_price(origin, kitty_id: T::KittyIndex, new_price: T::Balance) -> Result {
					let sender = ensure_signed(origin)?;
		...
		fn buy_kitty(origin, kitty_id: T::KittyIndex, max_price: T::Balance) -> Result {
            let sender = ensure_signed(origin)?;
...
impl<T: Trait> Module<T> {
	...
	fn transfer_from(from: T::AccountId, to: T::AccountId, kitty_id: T::KittyIndex) -> Result {
      ...
      ensure!(owner != to, "'from' account does not equ to");
